...
var nuxeo = http.createClient(8081, 'localhost');
...
function getRelations(docRef) {
var request = nuxeoRequest('Relations.GetRelations', {outgoing:true, predicate:true}, 'doc:' + docRef);
...
function getChildren(docRef) {
var request = nuxeoRequest('Document.GetChildren', {}, 'doc:' + docRef);
request.on('response', function (response) {
  console.log('STATUS: ' + response.statusCode);
  console.log('HEADERS: ' + JSON.stringify(response.headers));
  response.setEncoding('utf8');
...
request.on('response', function (response) {
  ...
  response.on('end', function() {
    ...
    var doc = eval('(' + res + ')');
    getBlob(doc.path);
    getChildren(doc.path);
    //getRelations(doc.path);
  });
});
}
