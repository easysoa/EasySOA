...
@Deploy({ 
    ...
    "org.nuxeo.frascati" })
public class FraSCAtiFeature
extends SimpleFeature implements WorkingDirectoryConfigurator
{

    protected static final Logger log = Logger.getLogger(FraSCAtiFeature.class
            .getCanonicalName());
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            frascatiTestLibsPath = new StringBuilder(testPath).append(sep)
                    .append("frascati").append(sep).append("lib").toString();
        ...
        log.info("nuxeo-frascati directory found : "
                + nuxeoFrascati.getAbsolutePath());

        String home = workingDir.getAbsolutePath();
        log.info("Default environment home path: " + home);

        File frascatiConfigDir = new File(new StringBuilder(home).append(sep)
        ...
        File frascatiDir = new File(new StringBuilder(
                nuxeoFrascati.getAbsolutePath()).append(sep)
                .append("resources").append(sep)
                .append("frascati").toString());

        File configFileSrc = new File(new StringBuilder(
                frascatiDir.getAbsolutePath()).append(sep)
                .append("config").append(sep)
                .append("frascati_boot.properties").toString());

        File configFileDst = new File(new StringBuilder(
                frascatiConfigDir.getAbsolutePath()).append(sep)
                .append("frascati_boot.properties").toString());
        try
        {    
        ...
        File frascatiLibsDir = new File(new StringBuilder(
                frascatiDir.getAbsolutePath()).append(File.separatorChar)
                .append("lib").toString());
        ...
        for (File srclib : libs)
        {
            ...
            File destlib = new File(new StringBuilder(frascatiTestLibsPath)
                    .append(sep).append(libName).toString());
            try
            {
            ...
            final String directoryName)
    {
        ...
        File[] children = basedir.listFiles(new FileFilter()
        {
            ...
            {
                if (f.isDirectory() &&
                       (caller == null || !(caller.getAbsolutePath().equals(
        ...
        if (caller == null || !caller.getAbsolutePath().equals(
                basedir.getAbsoluteFile().getParentFile().getAbsolutePath()))
        {
