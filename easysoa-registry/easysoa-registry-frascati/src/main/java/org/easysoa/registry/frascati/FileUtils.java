...
public class FileUtils {
	...
	private static Log log = LogFactory.getLog(FileUtils.class);
	  ...
	  /**
	   * Unzips the given jar in a temp file and returns its files' URLs 
	   * Inspired from AssemblyFactoryManager.processContribution()
	   * (though doesn't add it to the classloader or parse composites)
	   * 
	   * TODO move to util
	   * TODO better : delete temp files afterwards (or mark em so) OR rather use jar:url ?!
	   * 
	  ...
	  public static final Set<URL> unzipAndGetFileUrls(File file)
			  {
			    try {   
			      ...
			      final String folder = zipFile.getName().substring(
			          zipFile.getName().lastIndexOf(File.separator),
