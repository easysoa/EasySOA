...
import org.easysoa.proxy.core.api.template.TemplateEngineImpl;
...
public class ExchangeRecordProxyReplayTest extends AbstractProxyTestStarter {
	...
	private static Logger logger = Logger
			.getLogger(ExchangeRecordProxyReplayTest.class.getName());
	...
	public void testReplayWithRestMessages() throws Exception {
		...
		// Start a new Run
		HttpPost newRunPostRequest = new HttpPost(
				"http://localhost:8084/run/start/" + testStoreName);
		assertEquals("Run '" + testStoreName + "' started !",
				httpClient.execute(newRunPostRequest,
						new BasicResponseHandler()));

		// Get the twitter mock set and send requests to the mock through the
		// HTTP proxy
		...
		for (String url : urlMock.getTwitterUrlData("localhost:"
			...
			// Need to read the response body entierely to be able to send
			// another request
			entityResponseString = ContentReader.read(response.getEntity()
					.getContent());
		}

		// Stop and save the run
		HttpPost stopRunPostRequest = new HttpPost(
				"http://localhost:8084/run/stop");
		assertEquals("Current run stopped !", httpClient.execute(
				stopRunPostRequest, new BasicResponseHandler()));
		// delete the run
		HttpPost deleteRunPostRequest = new HttpPost(
				"http://localhost:8084/run/delete");
		assertEquals("Run deleted !", httpClient.execute(deleteRunPostRequest,
				new BasicResponseHandler()));

		// get a list of recorded exchange store
		httpUriRequest = new HttpGet("http://localhost:"
				+ EasySOAConstants.EXCHANGE_RECORD_REPLAY_SERVICE_PORT
				+ "/getExchangeRecordStorelist");
		response = httpClient.execute(httpUriRequest);
		entityResponseString = ContentReader.read(response.getEntity()
				.getContent());
		// logger.debug("Exchange record store list response : " +
		// entityResponseString);
		assertTrue(entityResponseString.contains(testStoreName));

		// Get an exchange record
		httpUriRequest = new HttpGet("http://localhost:"
				+ EasySOAConstants.EXCHANGE_RECORD_REPLAY_SERVICE_PORT
				+ "/getExchangeRecord/" + testStoreName + "/1");
		response = httpClient.execute(httpUriRequest);
		entityResponseString = ContentReader.read(response.getEntity()
				.getContent());
		// logger.debug("Exchange record response : " + entityResponseString);
		assertTrue(entityResponseString
				.contains("{\"exchangeRecord\":{\"exchange\":{\"exchangeID\":1"));

		// get a list of recorded exchanges contained in the Test_Run folder
		httpUriRequest = new HttpGet("http://localhost:"
				+ EasySOAConstants.EXCHANGE_RECORD_REPLAY_SERVICE_PORT
				+ "/getExchangeRecordList/" + testStoreName);
		// logger.debug("Sending request");
		response = httpClient.execute(httpUriRequest);
		// logger.debug("Reading response");
		entityResponseString = ContentReader.read(response.getEntity()
				.getContent());
		// logger.debug("Exchange record list response : " +
		// entityResponseString);
		...
		List<ExchangeRecord> recordList = fileStore
				.getExchangeRecordlist(testStoreName);

		// Send a request to the replay service
		for (ExchangeRecord record : recordList) {
			// ExchangeRecord record = recordList.get(0);
			originalResponse = record.getOutMessage().getMessageContent()
					.getRawContent();
			httpUriRequest = new HttpGet("http://localhost:"
					+ EasySOAConstants.EXCHANGE_RECORD_REPLAY_SERVICE_PORT
					+ "/replay/" + testStoreName + "/"
					+ record.getExchange().getExchangeID());
			response = httpClient.execute(httpUriRequest);
			entityResponseString = ContentReader.read(response.getEntity()
					.getContent());
			// logger.debug("Original ExchangeRecord response : " +
			// originalResponse);
			// logger.debug("Replayed ExchangeRecord response : " +
			// entityResponseString);
		...
		// Create a ReplaySequencer class to organise to execution of differents
		// engines
		// Add boolean variables in property file to enable or disable the
		// execution of engines
		ReplayEngine replayEngine = frascati.getService(componentList.get(0)
				.getName(), "replayEngineService",
				org.easysoa.proxy.core.api.records.replay.ReplayEngine.class);
		replayEngine.startReplaySession(testStoreName + "_replaySession");
		for (ExchangeRecord record : recordList) {
			// Field suggestions
			TemplateFieldSuggestions fieldSuggestions = templateEngine
					.suggestFields(record, testStoreName, true);
			// Assertions suggestions
			AssertionSuggestions assertionSuggestions = assertionEngine
					.suggestAssertions(fieldSuggestions, record.getExchange()
							.getExchangeID(), testStoreName);
			// templatized record
			templateEngine.generateTemplate(fieldSuggestions, record,
					testStoreName, true);
			// Replaying records
			...
			replayedMessageContent
					.setRawContent(replayEngine
							.replay(testStoreName,
									record.getExchange().getExchangeID())
							.getMessageContent().getRawContent());
			// TODO => call replayWithTemplate Method to test the remplate
			// engine

			// Executing assertions
			// TODO : Test for LCS assertion with a big message => Out of memory
			// error
			// Need to add a limitation to avoid to freeze or block the whole
			// test.
			// assertionSuggestions.addAssertion(new
			// LCSAssertion("LCSAssertion"));
			// assertionEngine.executeAssertions(assertionSuggestions,
			// record.getOutMessage(), replayedMessage);
		}
	...
	public void testReplayWithSoapMessages() throws Exception {
		...
		entityResponseString = ContentReader.read(response.getEntity()
				.getContent());
		// logger.info("Exchange record store list response : " +
		// entityResponseString);
		assertTrue(entityResponseString.contains(testStoreName));
		...
		for (ExchangeRecord record : recordList) {
			...
			// entityResponseString);
			// Compare the replayed exchange with the original exchange
			assertEquals(record.getOutMessage().getMessageContent()
					.getRawContent(), outMessage.getMessageContent()
					.getRawContent());
		}
	...
	protected static void cleanOldFiles() {
		String storePath;
		try {
			storePath = PropertyManager.getPropertyManager().getProperty(
					"path.record.store");
		} catch (Exception ex) {
			logger.error(
					"Unable to get 'path.record.store' property value, check PropertyManager init",
					ex);
