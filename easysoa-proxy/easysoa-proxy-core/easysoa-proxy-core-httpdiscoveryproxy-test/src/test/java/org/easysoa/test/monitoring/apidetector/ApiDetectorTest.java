...
package org.easysoa.test.monitoring.apidetector;
...
public class ApiDetectorTest extends TestCase
{
    ...
    public void testUrlDiscoveryMode()
    {
        ...
        {
            logger.warn("Failed to connect to Nuxeo, aborting test: "
                    + e.getMessage());
    ...
    public void testUrlValidatedMode()
    {
        ...
        try
        {
            ...
            urlDetectionSimulate(new UrlMock()
                    .getTwitterUrlData("api.twitter.com"));
        } catch (Exception e)
        {
            logger.warn("Failed to connect to Nuxeo, aborting test: "
                    + e.getMessage());
        ...
        // TODO LATER mixed mode : do compute and debugResults BUT ONLY on
        // unknown messages ?!
    }
    ...
    public void urlDetectionCompute()
    {
        ...
        // Not a singleton ...
        /*
         * if(MonitoringMode.DISCOVERY.compareTo(DiscoveryMonitoringService.
         * getMonitorService().getMode()) == 0){
         * DiscoveryMonitoringService.getMonitorService
         * ().registerDetectedServicesToNuxeo(); } else {
         * DiscoveryMonitoringService
         * .getMonitorService().registerUnknownMessagesToNuxeo(); }
         */
    }
    ...
    public void urlDetectionDebugResults()
    {
        ...
        // Not a singleton ...
        // UrlTree urlTree =
        // DiscoveryMonitoringService.getMonitorService().getUrlTree();

        /*
         * //UrlTree urlTree = null; if(urlTree != null){
         * logger.debug("Printing tree node index ***");
         * logger.debug("Total url count : " + urlTree.getTotalUrlCount());
         * String key; HashMap<String, UrlTreeNode> index =
         * urlTree.getNodeIndex(); Iterator<String> iter2 =
         * index.keySet().iterator(); UrlTreeNode node; UrlTreeNode parentNode;
         * while(iter2.hasNext()){ key = iter2.next(); node = index.get(key);
         * if(node.getLevel() <= 3){ parentNode =
         * (UrlTreeNode)(node.getParent()); logger.debug("complete = " +
         * node.getCompleteUrlcallCount() + ",partial = " +
         * node.getPartialUrlcallCount() + ",total = " +
         * urlTree.getTotalUrlCount()); logger.debug(key + " -- " +
         * node.toString() + ", parent node => " + parentNode.getNodeName() +
         * ", Depth => " + node.getDepth() + ", Level => " + node.getLevel());
         * logger.debug(", direct node childs => " + node.getChildCount() +
         * "Total childs number => " + node.getTotalChildsNumber());
         * logger.debug(", ratioPartial => " + node.getRatioPartial(urlTree) +
         * "%, ratioComplete => " + node.getRatioComplete(urlTree) + "%" +
         * ", Ratio childs => " + node.getRatioChilds() + "%" +
         * ", Ratio childs to ancestor => " + node.getRatioChildsToAncestor() +
         * "%"); } } }
         */
    }
