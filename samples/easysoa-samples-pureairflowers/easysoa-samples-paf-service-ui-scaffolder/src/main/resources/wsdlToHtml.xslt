...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- Base server URL -->
	<xsl:variable name="baseServerUrl">
		http://localhost:7001/?
	</xsl:variable>
	...
	<xsl:template name="fields">
		...
		<table>
			...
			<xsl:for-each
				select="key('complexTypes', $elementName)/xsd:sequence/xsd:element">
				<tr>
					<td>
	...
	<xsl:template match="wsdl:portType/wsdl:operation">
		...
		<script type="text/javascript">
			...
			//document.
			<xsl:value-of select="$operationName" />
			.ordersNumber.value = "Sending request ...";
			getElementsByClass('outputField', document.
			<xsl:value-of select="$operationName" />
			, '*')[0].value = "Sending request ...";
			...
			//document.
			<xsl:value-of select="$operationName" />
			.ordersNumber.value = "Getting response ...";
			getElementsByClass('outputField', document.
			<xsl:value-of select="$operationName" />
			, '*')[0].value = "Getting response ...";
			...
			//document.
			<xsl:value-of select="$operationName" />
			.ordersNumber.value = xhr.responseText;
			getElementsByClass('outputField', document.
			<xsl:value-of select="$operationName" />
			, '*')[0].value = xhr.responseText;
			} else {
			//document.
			<xsl:value-of select="$operationName" />
			.ordersNumber.value = "Error code " + xhr.status;
			getElementsByClass('outputField', document.
			<xsl:value-of select="$operationName" />
			, '*')[0].value = "Error code " + xhr.status;
			}
			}
			};
			...
			var url = "
			<xsl:value-of select="$baseServerUrl" />"
			var params = "";
		...
		<input type="button" value="Submit"
			OnClick="submit{$operationName}Form('{$operationName}');" />
	</xsl:template>
